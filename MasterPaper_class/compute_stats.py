...
def run_nuclei_inst_stat(pred_dir, true_dir, print_img_stats=True, ext=".mat"):
    ...
    with open(metric_save + "/" + "metrics.txt", 'a', encoding="utf-8", newline='') as outFile:
    ...
    for filename in file_list[:]:
        ...
        if print_img_stats:
            ...
            for scores in metrics:
                ...
                with open(metric_save + "/" + "metrics.txt", 'a', encoding="utf-8", newline='') as outFile:
                    print(basename + " %f " % scores[-1], end="  ", file=outFile)
            print()
...
if __name__ == "__main__":
    ...
    mode = "type"   # instance  # type
    dataset_name = "CoNSeP"
    train_mode_type = "_finetune_100"  # _finetune_100 _normal
    model_type = "Master"  # Master  noAtt  noSelf  noSelfnoAtt
    preprocessing_mode = "modistain"  # modistain  normalstain nopreprocessing
    other_infor = ""
    post_type = ""  # _post_new  _post_old

    for run_no in range(1):
        folder_name = "Exam001_" + dataset_name + "_" + model_type + "_" + preprocessing_mode + train_mode_type + "_" + other_infor + str(run_no)
